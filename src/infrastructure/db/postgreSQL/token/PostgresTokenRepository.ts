...
export class PostgresTokenRepository implements TokenRepositoryInterface {
    async createToken (userId: string, refreshToken: string){
        const tokenDocument = await prisma.token.create({data: {userId: Number(userId), refreshToken}})
        const tokenEntity: TokenEntity = mapper.toEntity(tokenDocument)
        return tokenEntity
    }

    async findTokenByUser (userId: string){
        const tokenDocument: TokenDocument | null = await prisma.token.findUnique({where: {userId: Number(userId)}})
    ...
    async saveToken (tokenId: string, refreshToken: string){
        const tokenDocument: TokenDocument | null= await prisma.token.update({where:{id: Number(tokenId)}, data: {refreshToken}})
    ...
    async deleteToken (userId: string){
        await prisma.token.delete({where: {userId: Number(userId)}})
        return
    }
}